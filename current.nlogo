globals [ Smin Smax distWeight colorWeight ]
turtles-own [ S Ac maxAcVal stuck? nextStep ]
...
to setup
  ...
  ask n-of population patches with [pxcor < max-pxcor - 10 ][
    sprout 1 [
...
to go
  ask turtles [
    ...
    ifelse any? empty-patches
      [ let target min-one-of empty-patches [ distance patch 28 0 ]
        face target
        move-to target ]
      [ if is-agent? Ac and [xcor] of Ac > [xcor] of self [
          face Ac
          move-to Ac ]
      ]
      
      if patch-here = patch 28 0 and not any? turtles-on patch -28 0 [ move-to one-of patches with [pxcor = -28] ]
...
to-report Sfdistance [Ame Ao]
    ifelse euclidean-distance [xcor] of Ame [ycor] of Ame [xcor] of Ao [ycor] of Ao <= 0
    [ report 0 ]
